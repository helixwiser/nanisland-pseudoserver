...
process_sample() {
    ...
    fastp -i $folder_name.1_val_1.fq.gz -I $folder_name.2_val_2.fq.gz \
      -o $folder_name.fastp.1.fastq -O $folder_name.fastp.2.fastq \
      --cut_right \
      --cut_right_window_size 4 \
      --cut_right_mean_quality 20 \
      --thread 8
    
    local readt1=$folder_name.fastp.1.fastq
    local readt2=$folder_name.fastp.2.fastq
    ...
    if [ "$host" = "human" ]; then
        ...
        samtools stats aligned_hm.sorted.bam > stats.aligned_hm.log
        ...
        cp stats.aligned_hm.log $trimdir/result01_hostalignment/${folder_name}_samtoolstat.log
        cp stats.reads.aligned_hm.log $trimdir/result01_hostalignment/${folder_name}_samtoolflagstat.txt
        cp stats.cover.mapped_hm.log $trimdir/result01_hostalignment/${folder_name}_samtoolcover.txt
        cp aligned_hm_marked_dup_metrics.txt $trimdir/result01_hostalignment/${folder_name}_dup_metrics.txt
    fi
    ...
    if [ "$host" = "mouse" ]; then
        ...
        samtools stats aligned_mm.sorted.bam > stats.aligned_mm.log
        ...
        cp stats.aligned_mm.log $trimdir/result01_hostalignment/${folder_name}_samtoolstat.log
        cp stats.reads.aligned_mm.log $trimdir/result01_hostalignment/${folder_name}_samtoolflagstat.txt
        cp stats.cover.mapped_mm.log $trimdir/result01_hostalignment/${folder_name}_samtoolcover.txt
        cp aligned_mm_marked_dup_metrics.txt $trimdir/result01_hostalignment/${folder_name}_dup_metrics.txt
    ...
    cp *fastqc.html $trimdir/result01_hostalignment/.
    cp *trimming_report.txt $trimdir/result01_hostalignment/.
    
    
    
    conda activate py39
    cd $trimdir/result01_hostalignment/
    multiqc .
...
for file in "${files[@]}"; do 
    process_file $file $source_dir $destination_dir >> $destination_dir/s1joblog.txt 2>&1
done
