...
#SBATCH -p intel-sc3,amd-ep2,intel-sc3-32c,20240429
#SBATCH -q normal
...
#for uniprot_file in "$INPUT_DIR"/*cancer-skinread_uniprot.[1].tsv; do
...
#    base_name=$(basename "$uniprot_file" | sed -E 's/_uniprot\.[1]\.tsv$//')
#
#
...
#
#
#echo "Allca jobs completed."


processdir="/storage/caishangLab/fangxiunan/ASAP/test260121-clustercore/no1_foldseek"
rm -rf $processdir/no_skeleton_membercount && mkdir -p $processdir/no_skeleton_membercount
cd $processdir/no_skeleton_membercount
# Define fixed inputs
SKELETON_EDGE=$processdir/fa_layouttest/skeleton_edges.csv
CLUSTER_FILE=$processdir/cluster_data/foldseek-res_cluster.tsv
INPUT_DIR="/storage/caishangLab/fangxiunan/project-ongoing/metagenomic/maf_processedresult"
...
# Loop over all _uniprot.1.tsv and _uniprot.2.tsv files
for uniprot_file in "$INPUT_DIR"/*normal-skinread_uniprot.[1].tsv; do
    if [[ ! -f "$uniprot_file" ]]; then
        continue  # Skip if no files match
    fi
    ...
    # Define output filename
    output_file="${base_name}_no_skeleton_membercount.csv"
